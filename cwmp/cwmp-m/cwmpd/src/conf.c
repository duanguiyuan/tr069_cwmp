...
void cwmp_conf_init(cwmp_t * cwmp)
{
    ...
    cwmp->cpe_PeriodicInformEnable = cwmp_conf_get_int("cwmp:cpe_perodicEnable");
	
	if(cwmp->cpe_PeriodicInformEnable)
	{
		cwmp->cpe_PeriodicInformInterval= cwmp_conf_get_int("cwmp:cpe_perodicInterval");
...
void get_cwmp_event_filename(cwmp_t * cwmp)
{
	char buf[128]={0};
	cwmp_conf_get("cwmp:event_filename", buf);
	cwmp->event_filename = strdup(buf);
	//cwmp_log_debug("buf = %s  filename = %s ",buf,cwmp->event_filename);
}