...
void cwmp_agent_start_session(cwmp_t * cwmp)
{
    ...
    while (TRUE)
    {	/* 添加心跳检测*/
    	if(cwmp->cpe_PeriodicInformEnable == 1)
    	{
    		timeTmp = cwmp_conf_uptime_get();
			if((timeTmp - cwmp->cpe_time) > cwmp->cpe_PeriodicInformInterval) 
			{
				//cwmp_log_debug("cwmp Inform Periodic time = %u\n",timeTmp);
				cwmp->cpe_time = timeTmp-2; /* 这里的2 取自while循环的轮询周期*/
				cwmp->new_request = CWMP_YES;
				cwmp_log_debug("set cwmp new request to Inform Periodic\n");
        ...
        while (!session_close)
        {
            ...
            case CWMP_ST_SEND://发送报文
                ...
                break;
            case CWMP_ST_RECV://接收报文 并 分析http头
                ...
                else
                {
                	...
                	/*change by duanguiyuan 20160608*/
					/* if(session->last_code == 401 ||session->last_code == 407)
					 {
					 	session->status = CWMP_ST_INFORM;
					 }else{
						session->status = CWMP_ST_END;
            ...
            case CWMP_ST_ANSLYSE://分析报文的 soap内容
...
int cwmp_agent_download_file(download_arg_t * dlarg)
{
    ...
    char * tofile;
	if(atoi(dlarg->filetype)==1){
		//升级
		tofile = "/tmp/app.bin";
	}else if(atoi(dlarg->filetype)==3){
		//下发配置
		tofile = "/tmp/config.txt";
	}else{
		//未知
		tofile = "/tmp/dowmload.img";
	}
    ...
    faultcode = http_receive_file(fromurl, tofile);
 	cwmp_log_debug("upload file fail:  %d", faultcode);
    if(faultcode == CWMP_ERROR)
    {
	     faultcode = 9001;
    }else{
		faultcode == CWMP_OK;
	}
   	/* 接收完文件 之后的  使用 vflag localupdate 升级  或者 恢复配置 到 nvram 中*/
    return faultcode;
...
int cwmp_agent_upload_file(upload_arg_t * ularg)
{
    ...
    FUNCTION_TRACE();
    char * fromfile;
	if( atoi(ularg->filetype) == 1)
	{
		//根据实际情况, 修改这里的配置文件路径
		system("nvram show|grep = > /tmp/system_nvram.cfg");
		fromfile = "/tmp/system_nvram.cfg";

	}else if(atoi(ularg->filetype) == 2){
		//根据实际情况, 修改这里的配置文件路径
		system(" cat  /var/log/messages.txt > /tmp/log.txt");
		fromfile = "/tmp/log.txt";
			...
			case TASK_DOWNLOAD_TAG:
				{
					...
					faultcode = cwmp_agent_download_file(dlarg);
				 	cwmp_log_debug("cwmp_agent_run_tasks() upload file fail:  %d", faultcode);	
					time_t endtime = time(NULL);
					if(faultcode == CWMP_OK)
					{
						if(atoi(dlarg->filetype)==1){
							system("vflag localupdate ");
							system("reboot 5 by cwmp update version &");
						}
					}else{
						cwmp_log_debug("download file error ...");
			...
			case TASK_REBOOT_TAG:
				{
					...
					cwmp_event_clear_active(cwmp);
					system("reboot 5 by cwmp reboot &");
				}
				break;
			...
			case TASK_FACTORYRESET_TAG:
				{
					...
					cwmp_event_clear_active(cwmp);
					system("nvram set factory_recover=1");
					system("nvram commit");
					system("reboot 5 by cwmp factory &");
				}
				break;
