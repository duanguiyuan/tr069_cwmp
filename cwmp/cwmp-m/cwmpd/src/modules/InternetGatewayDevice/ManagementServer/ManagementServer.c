...
int cpe_get_igd_ms_connectionrequesturl(cwmp_t * cwmp, const char * name, char ** value, pool_t * pool)
{
    ...
    char local_ip[32]={0};
	char *wanport;
	/* 获取wan口的 端口 eth0  或者 br0 */
	wanport = cwmp_conf_pool_get(pool, "cwmpd:wan_port");
    cpe_get_localip(wanport, local_ip);
...
{    
   *value = cwmp_conf_pool_get(pool, "cwmp:cpe_perodicEnable");
   
    cwmp_log_debug("cpe_get_igd_ms_periodicInformEnable: value is %s", *value);
    //	*value=strdup("1");
    return FAULT_CODE_OK;
}
...
{
	cwmp_conf_set("cwmp:cpe_perodicEnable", value);
	cwmp->cpe_PeriodicInformEnable = atoi(value);
	/* 如果 开启 心跳上报 重置计时时间*/
	if(cwmp->cpe_PeriodicInformEnable == 1)
	{
		cwmp->cpe_time = cwmp_conf_uptime_get();
	}
	cwmp_log_debug("cpe_set_igd_ms_periodicInformEnable: value is %d", cwmp->cpe_PeriodicInformEnable );
    return FAULT_CODE_OK;
}
...
{    
    *value = cwmp_conf_pool_get(pool, "cwmp:cpe_perodicInterval");
	cwmp_log_debug("cpe_get_igd_ms_PeriodicInformInterval: value is %s", *value);
   //	*value=strdup("300");
    return FAULT_CODE_OK;
}
...
{
	cwmp_conf_set("cwmp:cpe_perodicInterval", value);
	cwmp->cpe_PeriodicInformInterval = atoi(value);
	cwmp_log_debug("cpe_set_igd_ms_PeriodicInformInterval: value is %d", cwmp->cpe_PeriodicInformInterval );
    return FAULT_CODE_OK;
}

//InternetGatewayDevice.ManagementServer.PeriodicInformTime
int cpe_get_igd_ms_PeriodicInformTime(cwmp_t * cwmp, const char * name, char ** value, pool_t * pool)
{    
   // *value = cwmp_conf_pool_get(pool, "cwmp:cpe_password");
   	*value=strdup("0000-00-00T00:00:00");
    return FAULT_CODE_OK;
}
int cpe_set_igd_ms_PeriodicInformTime(cwmp_t * cwmp, const char * name, const char * value, int length, callback_register_func_t callback_reg)
{
	//cwmp_conf_set("cwmp:cpe_password", value);
    return FAULT_CODE_OK;
}
