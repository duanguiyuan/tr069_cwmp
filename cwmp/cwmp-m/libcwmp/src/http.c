...
int http_read_body(http_socket_t * sock, int max)//, cwmp_chunk_t * body, pool_t * pool)
{
    ...
    while (bytes < max)
    {
        ...
        //memcpy(b, buffer, len);
		//cwmp_log_error("recv, buf = %d ",strlen(buffer));
...
int http_read_response(http_socket_t * sock, http_response_t * response, pool_t * pool)
{
    ...
    version = line[0];
	cwmp_log_debug("http line[0] = %s", version);
    for (i=0; i < slen; i++)
    {
        if (line[0][i] == ' ')
        {
	...
	cwmp_log_debug("http version = %s respCode = %s message = %s", version,resp_code,message);
    if (version == NULL || resp_code == NULL || message == NULL)
    {
    	cwmp_log_debug("http error = %s", version);
        return CWMP_ERROR;
    }
	...
	cwmp_log_debug("http code : %d body length %d, [\n%s\n]", code, cont_len);
    rc = http_read_body(sock, cont_len);//, &body, pool);
    cwmp_log_debug("http read body resualt = %d", rc);
    if (rc < 0 || code != 200)
    {
...
int http_write_request(http_socket_t * sock , http_request_t * request, cwmp_chunk_t * chunk, pool_t * pool)
{
   ...
   //      if((dest->auth.active == CWMP_FALSE) && (dest->auth_type == HTTP_DIGEST_AUTH))
	if(dest->auth_type == HTTP_DIGEST_AUTH)
   		{
...
int http_send_file_request(http_socket_t * sock , http_request_t * request, const char  * fromfile, pool_t * pool)
{
	...
	/* 获取 文件的 大小 */
	cwmp_log_debug("upload file path = %s size = %d",fromfile,len2);
    FILE *tf = fopen(fromfile, "rb");
    if(!tf)
    {
    	cwmp_log_error("http upload file can't open ");
		return CWMP_ERROR;
    }
	...
	/*添加  post 头*/
    totallen = len1;
	cwmp_log_error("http upload file totallen = %d ",totallen);
    while(1)
    {
    	memset(buffer,0,sizeof(buffer));
		len2 = fread(buffer, HTTP_DEFAULT_LEN, 1, tf);
	//	cwmp_log_error("http upload file len2 = %d : %s ",len2,buffer);
		if((len2 <= 0)&&(strlen(buffer)<2))
		{
			break;
		}
		
		len2 = http_socket_write(sock, buffer, strlen(buffer));
		if(len2 <= 0)
		{
			break;
		}
		totallen += len2;
		cwmp_log_error("http upload file while totallen = %d ",totallen);
    }
